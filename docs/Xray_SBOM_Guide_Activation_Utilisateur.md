...
### 3. Réagir vite en cas de faille — avec « Impact Search »
...
L'activation du SBOM débloque une fonctionnalité dédiée dans l'interface : **Impact Search**. Elle permet d'évaluer le rayon d'impact (« blast radius ») d'un problème de sécurité selon trois modes de recherche :

- **Par vulnérabilité** : saisir une CVE (ou un XRAY ID) pour lister tous les artefacts affectés.
- **Par version de paquet** : trouver les artefacts contenant une version précise d'un composant.
- **Par paquet (toutes versions)** : trouver toutes les traces d'un composant, quelle que soit sa version.

**Accès :** Xray > Scans List > Impact Search (à partir de Xray 3.137.16).
...
- **Périmètre :** ce guide couvre l'analyse des dépendances open source (SCA). La vue arborescente interactive des dépendances et l'analyse d'exploitabilité (Contextual Analysis) relèvent d'un module (Advanced Security) non disponible dans notre licence ; elles ne sont donc pas accessibles. En revanche, l'Impact Search (recherche des artefacts impactés) et l'export enrichi des dépendances, eux, le sont : ils dépendent du service SBOM, pas d'Advanced Security.
...
**Comment retrouver les artefacts touchés par une CVE ?**
Via Impact Search (Xray > Scans List > Impact Search), une fois le SBOM activé. Recherchez par CVE, par paquet ou par version.
